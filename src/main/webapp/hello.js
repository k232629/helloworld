...
function enableButtons () {
    ...
    // Update the button label now that the button is active
    btn.value="Click me for a timing greeting";

    btn = document.getElementById("input_greet_by_mood");
    btn.onclick=function(){greetByMood();};

    // Update the button label now that the button is active
    btn.value="Click me to know a mood";
}
...
function greetByMood(){
    var name = document.getElementById("name_field").value;
    var period = document.getElementById("period_field").value;
    var mood = document.getElementById("mood_field").value;
    console.log(mood);
    var request = gapi.client.helloworldendpoints.greetByMood({'name': name, 'period':period , 'mood':mood});
    request.execute(sayHelloCallback);
}
